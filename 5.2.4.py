...
v_final_list,v_user_attribute_list, v_app_list, v_app_inlog_list, v_g_user_attribute_list, v_g_app_list, v_g_app_inlog_list = [], [], [], [],  [], [], []
...
def get_full_missing_view_3D(used_ids, ys, v_list, use_PCA=True):
    ...
    for i, used_id in enumerate(used_ids):
        if used_id in user_info_set and used_id in install_behave_set and         used_id in install_list_set and used_id in user_log_set:
    ...
    return v_low_full_sample_positive, v_low_full_sample_negative, v_low_missing_sample_positive,                            v_low_missing_sample_negative
...
ys = df_master_records['target_1m30+'].loc[used_ids].values
logging.info('start')
v_low_full_sample_positive, v_low_full_sample_negative, v_low_missing_sample_positive, v_low_missing_sample_negative = get_full_missing_view_3D(used_ids, ys, v_final_list, True)
logging.info('end')
...
ys = df_master_records['target_1m30+'].loc[used_ids].values
v_final_low_positive, v_final_low_negative, v_origin_low_positive, v_origin_low_negative = get_missing_sample_vfinal_vorigin(missing_sample_idxs, ys, v_final_list, v_origin_list, True)
