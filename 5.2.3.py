...
#         ], index = ['test', 'behave', 'list', 'user_info', 'user_log'])
        ...
        if max_auc < test_ret_dict['1m30+']:
            ...
            os.system('rm 3_data/ret_%s*'%use_view_name)
            os.system('rm 3_data/model_%s*'%use_view_name)
            
            df_ret.to_csv('3_data/ret_%s_epoch_%d.csv' % (use_view_name, epoch), index=False)
            torch.save(model, '3_data/model_%s_epoch_%d.torch.model' % (use_view_name, epoch))
